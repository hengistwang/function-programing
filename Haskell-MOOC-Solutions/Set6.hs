...
instance Ord Country where
  compare x y
    | x == y = EQ
    | otherwise = if x == Finland || y == Switzerland
      then LT
      else if x == Switzerland || y == Finland
        then GT
        else LT
...
instance Eq a => Eq (List a) where
  ...
  (==) _ Empty = False
  (==) Empty _ = False
  (==) (LNode val1 next1) (LNode val2 next2) = val1 == val2 && next1 == next2
...
instance Num RationalNumber where
  ...
  abs (RationalNumber x y) = simplify (RationalNumber (abs x) (abs y))
