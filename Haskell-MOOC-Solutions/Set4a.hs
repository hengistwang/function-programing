...
longest xs = go [] xs
  where
    go ans [] = ans
    go ans (x : xs) =
      if length ans < length x || (length ans == length x && head ans > head x)
        then go x xs
        else go ans xs
...
incrementKey k = map (\(key, value) -> if key == k then (key, value + 1) else (key, value))
...
freqs [] = Map.empty
freqs (key : xs) = Map.alter freqs' key $ freqs xs
  where
...
transfer from to amount bank =
  if check1 from to bank && check2 from amount bank
    then Map.adjust (+ amount) to (Map.adjust (\x -> x - amount) from bank)
    else bank
  where
    check1 from to bank = Map.member from bank && Map.member to bank
    check2 from amount bank = amount > 0 && Map.findWithDefault 0 from bank - amount >= 0
...
maxIndex m = fst (go (assocs m) (head (assocs m)))
  where
    go [(i, v)] max = if v > snd max then (i,v) else max
    go ((i, v) : xs) max = if v > snd max then go xs (i, v) else go xs max
