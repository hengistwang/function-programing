...
oddFactorial x = if even x then oddFactorial' (x-1) else oddFactorial' x
    where
        oddFactorial' 1 = 1
...
leftpad str n = replicate (n - length str) ' ' ++ str
...
countdown x = "Ready! "++ help x ++" Liftoff!"
    where
        help 0 = ""
        help x = show x ++ "... " ++ help (x - 1)
...
smallestDivisor n = help n 2
    where
        ...
        help n x = if n `mod` x == 0 then x else help n (x + 1)
