...
firstHalf x = if even (length x) then take (length x `div` 2) x else take ((length x `div` 2) + 1) x
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) [k ^ i | i <- [0 .. (max `div` k)]]
...
joinToLength n str = [target | first <- str, last <- str, let target = first ++ last, length target == n]
...
-- sumRights arr = sum (map f arr)
--   where
--     f (Left _) = 0
--     f (Right x) = x

sumRights [] = 0
sumRights (x : arr) = f x + sumRights arr
  where
    f (Left _) = 0
    f (Right a) = a
...
multiCompose [] a = a
multiCompose (x : xs) a = x (multiCompose xs a)
...
interpreter commands = interpreter' commands 0 0 []
  where
    ...
    interpreter' ("up" : commands) x y list = interpreter' commands x (y + 1) list
    interpreter' ("down" : commands) x y list = interpreter' commands x (y - 1) list
    interpreter' ("left" : commands) x y list = interpreter' commands (x - 1) y list
    interpreter' ("right" : commands) x y list = interpreter' commands (x + 1) y list
    interpreter' ("printX" : commands) x y list = interpreter' commands x y (list ++ [show x])
    interpreter' ("printY" : commands) x y list = interpreter' commands x y (list ++ [show y])
